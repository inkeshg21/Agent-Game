...
public class ACOTesterAgent2 : MonoBehaviour
{
    // The ACO Controller.
    ...
    private List<ACOConnections> MyRoute2 = new List<ACOConnections>();
    ...
    void Start()
    {
        ...
        // Run ACO
        MyRoute2 = MyACOCON.ACO(iterationThreshold, totalNumAnts, Waypoints.ToArray(), Connections, StartNode, MaxPathLength);
        initialspeed = speed;
    ...
    void OnDrawGizmos()
    {
        ...
        if (MyRoute2.Count > 0)
        {
            foreach (ACOConnections aConnection in MyRoute2)
            {
    ...
    void Update()
    {
        if (MyRoute2.Count > 0)
        {
            if (transform.position != MyRoute2[current].GetToNode().transform.position)
        {
            Vector3 pos2 = Vector3.MoveTowards(transform.position, MyRoute2[current].GetToNode().transform.position, speed * Time.deltaTime);
            var LookPos = MyRoute2[current].GetToNode().transform.position - transform.position;
            LookPos.y = 0;
        ...
        {
            current = (current + 1) % ((MyRoute2.Count));


            // if (current + 2 == (ConnectionArray.Count - 1) && (transform.position != ConnectionArray[current].GetToNode().transform.position))
            if (current + (MyRoute2.Count - 1) == (MyRoute2.Count - 1) && (transform.position != MyRoute2[current].GetToNode().transform.position))
            {
                if ((transform.position != MyRoute2[(current + (MyRoute2.Count - 1))].GetFromNode().transform.position))
                {
                    countit = countit + 1;
                    // MyRoute2.Reverse();
                    if(countit==1)
                    {
                        this.GetComponent<PathfindingTesterAgent2>().enabled = true;
            ...
            {
                {
                    current = (current) % ((MyRoute2.Count));
                }
